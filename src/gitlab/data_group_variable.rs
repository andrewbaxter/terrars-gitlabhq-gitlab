...
struct DataGroupVariable_ {
    shared: StackShared,
    tf_id: String,
    data: RefCell<DataGroupVariableData>,
}

#[derive(Clone)]
pub struct DataGroupVariable(Rc<DataGroupVariable_>);

impl DataGroupVariable {
...
impl Referable for DataGroupVariable {
    fn extract_ref(&self) -> String {
        format!("data.{}.{}", self.0.extract_datasource_type(), self.0.extract_tf_id())
    }
}

impl Datasource for DataGroupVariable { }

impl ToListMappable for DataGroupVariable {
    type O = ListRef<DataGroupVariableRef>;
...
impl Datasource_ for DataGroupVariable_ {
...
pub struct BuildDataGroupVariable {
    pub tf_id: String,
...
impl BuildDataGroupVariable {
    pub fn build(self, stack: &mut Stack) -> DataGroupVariable {
        let out = DataGroupVariable(Rc::new(DataGroupVariable_ {
            shared: stack.shared.clone(),
            tf_id: self.tf_id,
            data: RefCell::new(DataGroupVariableData {
...
pub struct DataGroupVariableRef {
    shared: StackShared,
    base: String,
}

impl Ref for DataGroupVariableRef {
