...
struct GroupCluster_ {
    shared: StackShared,
    tf_id: String,
    data: RefCell<GroupClusterData>,
}

#[derive(Clone)]
pub struct GroupCluster(Rc<GroupCluster_>);
...
impl Referable for GroupCluster {
    fn extract_ref(&self) -> String {
        format!("{}.{}", self.0.extract_resource_type(), self.0.extract_tf_id())
    }
}

impl Resource for GroupCluster { }

impl ToListMappable for GroupCluster {
    type O = ListRef<GroupClusterRef>;
...
impl Resource_ for GroupCluster_ {
    fn extract_resource_type(&self) -> String {
        "gitlab_group_cluster".into()
    }
...
pub struct BuildGroupCluster {
    pub tf_id: String,
    #[doc= "The id of the group to add the cluster to."]
...
impl BuildGroupCluster {
    pub fn build(self, stack: &mut Stack) -> GroupCluster {
        let out = GroupCluster(Rc::new(GroupCluster_ {
            shared: stack.shared.clone(),
            tf_id: self.tf_id,
            data: RefCell::new(GroupClusterData {
...
pub struct GroupClusterRef {
    shared: StackShared,
    base: String,
}

impl Ref for GroupClusterRef {
