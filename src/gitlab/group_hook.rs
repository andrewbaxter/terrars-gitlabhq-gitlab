...
struct GroupHook_ {
    shared: StackShared,
    tf_id: String,
    data: RefCell<GroupHookData>,
}

#[derive(Clone)]
pub struct GroupHook(Rc<GroupHook_>);
...
impl GroupHook {
    ...
    pub fn set_subgroup_events(self, v: impl Into<PrimField<bool>>) -> Self {
        self.0.data.borrow_mut().subgroup_events = Some(v.into());
        self
    }
...
impl Referable for GroupHook {
    fn extract_ref(&self) -> String {
        format!("{}.{}", self.0.extract_resource_type(), self.0.extract_tf_id())
    }
}

impl Resource for GroupHook { }

impl ToListMappable for GroupHook {
    type O = ListRef<GroupHookRef>;
...
pub struct BuildGroupHook {
    pub tf_id: String,
    #[doc= "The ID or full path of the group."]
    pub group: PrimField<String>,
    #[doc= "The url of the hook to invoke."]
    pub url: PrimField<String>,
}

impl BuildGroupHook {
    pub fn build(self, stack: &mut Stack) -> GroupHook {
        let out = GroupHook(Rc::new(GroupHook_ {
            shared: stack.shared.clone(),
            tf_id: self.tf_id,
            data: RefCell::new(GroupHookData {
...
pub struct GroupHookRef {
    shared: StackShared,
    base: String,
}

impl Ref for GroupHookRef {
