...
struct IntegrationJira_ {
    shared: StackShared,
    tf_id: String,
    data: RefCell<IntegrationJiraData>,
}

#[derive(Clone)]
pub struct IntegrationJira(Rc<IntegrationJira_>);
...
impl Referable for IntegrationJira {
    fn extract_ref(&self) -> String {
        format!("{}.{}", self.0.extract_resource_type(), self.0.extract_tf_id())
    }
}

impl Resource for IntegrationJira { }

impl ToListMappable for IntegrationJira {
    type O = ListRef<IntegrationJiraRef>;
...
impl Resource_ for IntegrationJira_ {
    fn extract_resource_type(&self) -> String {
        "gitlab_integration_jira".into()
    }
...
pub struct BuildIntegrationJira {
    pub tf_id: String,
...
impl BuildIntegrationJira {
    pub fn build(self, stack: &mut Stack) -> IntegrationJira {
        let out = IntegrationJira(Rc::new(IntegrationJira_ {
            shared: stack.shared.clone(),
            tf_id: self.tf_id,
            data: RefCell::new(IntegrationJiraData {
...
pub struct IntegrationJiraRef {
    shared: StackShared,
    base: String,
}

impl Ref for IntegrationJiraRef {
