...
struct ServiceSlack_ {
    shared: StackShared,
    tf_id: String,
    data: RefCell<ServiceSlackData>,
}

#[derive(Clone)]
pub struct ServiceSlack(Rc<ServiceSlack_>);
...
impl Referable for ServiceSlack {
    fn extract_ref(&self) -> String {
        format!("{}.{}", self.0.extract_resource_type(), self.0.extract_tf_id())
    }
}

impl Resource for ServiceSlack { }

impl ToListMappable for ServiceSlack {
    type O = ListRef<ServiceSlackRef>;
...
impl Resource_ for ServiceSlack_ {
    fn extract_resource_type(&self) -> String {
        "gitlab_service_slack".into()
    }
...
impl BuildServiceSlack {
    pub fn build(self, stack: &mut Stack) -> ServiceSlack {
        let out = ServiceSlack(Rc::new(ServiceSlack_ {
            shared: stack.shared.clone(),
            tf_id: self.tf_id,
            data: RefCell::new(ServiceSlackData {
...
pub struct ServiceSlackRef {
    shared: StackShared,
    base: String,
}

impl Ref for ServiceSlackRef {
